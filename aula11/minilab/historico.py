...
def cadastrar(nome: str, origem: str, tipo: str, num_tokens: int | None = None) -> Documento:
    """Cadastra um novo documento com status 'pendente'."""
    with Session(engine) as session:
        doc = Documento(nome=nome, origem=origem, tipo=tipo, num_tokens=num_tokens)
        session.add(doc)
        session.commit()
        session.refresh(doc)
        return doc
...
def main() -> None:
    ...
    # Histórico completo com SQL direto
    print('\nHistórico completo (SQL direto):')
    with Session(engine) as session:
        resultado = session.query(Documento).order_by(Documento.criado_em.desc()).all()
