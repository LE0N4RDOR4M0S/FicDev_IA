...
def gerar_dataset(n: int, f: int) -> np.ndarray:
	...
	parte1 = np.random.randn(n, f // 2) * 100
	parte2 = np.random.rand(n, f - f // 2) * 0.01
...
def benchmark_soma(n: int, repeticoes: int) -> dict:
	...
	lista_py = arr_np.tolist()

	t_numpy = timeit.timeit(stmt=lambda: arr_np.sum(), number=repeticoes) / repeticoes
	t_lista = timeit.timeit(stmt=lambda: sum(lista_py), number=repeticoes) / repeticoes
	return {"numpy_s": t_numpy, "lista_s": t_lista, "speedup": t_lista / t_numpy}
...
def benchmark_operacao_vetorial(n: int, repeticoes: int) -> dict:
	...
	t_numpy = timeit.timeit(stmt=lambda: a_np * b_np, number=repeticoes) / repeticoes
